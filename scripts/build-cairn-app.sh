...
configuration="debug"
arch_mode="native"
codesign_mode="${CAIRN_CODESIGN_MODE:-auto}"
codesign_identity="${CAIRN_CODESIGN_IDENTITY:-}"
codesign_keychain="${CAIRN_CODESIGN_KEYCHAIN:-}"

usage() {
  cat <<'EOF'
Usage: ./scripts/build-cairn-app.sh [debug|release] [--configuration debug|release] [--arch native|arm64|x86_64|universal]

Examples:
  ./scripts/build-cairn-app.sh debug
  ./scripts/build-cairn-app.sh --configuration release --arch universal

Environment:
  CAIRN_CODESIGN_MODE=auto|identity|adhoc|none
  CAIRN_CODESIGN_IDENTITY="Developer ID Application: Example, Inc. (TEAMID)"
  CAIRN_CODESIGN_KEYCHAIN=/path/to/signing.keychain-db
EOF
}
...
if [[ "${codesign_mode}" != "auto" && "${codesign_mode}" != "identity" && "${codesign_mode}" != "adhoc" && "${codesign_mode}" != "none" ]]; then
  echo "Unsupported CAIRN_CODESIGN_MODE: ${codesign_mode}" >&2
  exit 1
fi

read_package_version() {
  python3 - "${repo_root}/plugins/cairn/.cursor-plugin/plugin.json" <<'PY'
import json
import sys
...
build_binary() {
  ...
  swift build "${args[@]}" --product Cairn >&2
  printf '%s/Cairn\n' "${binary_dir}"
}
  ...
  case "${codesign_mode}" in
    ...
    identity)
      if [[ -z "${codesign_identity}" ]]; then
        echo "CAIRN_CODESIGN_IDENTITY is required when CAIRN_CODESIGN_MODE=identity" >&2
        exit 1
      fi
      printf '%s\n' "${codesign_identity}"
      return 0
...
codesign_app_bundle() {
  ...
  if ! identity="$(resolve_codesign_identity)"; then
    echo "Skipping codesign for ${app_path} (CAIRN_CODESIGN_MODE=none)" >&2
    return
  fi
...
bundle_version="${CAIRN_BUNDLE_VERSION:-$(git -C "${repo_root}" rev-list --count HEAD 2>/dev/null || echo 1)}"
release_app_bundle_name="Cairn.app"
development_app_bundle_name="Cairn (Dev).app"
legacy_app_bundle_name="Cairn.app"
legacy_dev_app_bundle_name="Cairn (Dev).app"
bundle_icon_name="Cairn.icns"
icon_master_png="${repo_root}/assets/app-icons/cairn-1024.png"
...
bundle_display_name="Cairn"
bundle_identifier="com.powerbeef.cairn"
app_variant="release"
app_bundle_name="${release_app_bundle_name}"

if [[ "${configuration}" != "release" ]]; then
  bundle_display_name="Cairn (Dev)"
  bundle_identifier="com.powerbeef.cairn.dev"
  app_variant="dev"
...
legacy_dev_app_root="${repo_root}/dist/${legacy_dev_app_bundle_name}"
contents_dir="${app_root}/Contents"
macos_dir="${contents_dir}/MacOS"
resources_dir="${contents_dir}/Resources"

rm -rf "${app_root}" "${legacy_app_root}" "${legacy_dev_app_root}"
...
case "${arch_mode}" in
  native)
    cp "$(build_binary "" "")" "${macos_dir}/Cairn"
    ;;
  arm64)
    cp "$(build_binary "arm64-apple-macosx14.0" ".build/arm64-${configuration}")" "${macos_dir}/Cairn"
    ;;
  x86_64)
    cp "$(build_binary "x86_64-apple-macosx14.0" ".build/x86_64-${configuration}")" "${macos_dir}/Cairn"
    ;;
  universal)
    ...
    lipo -create -output "${macos_dir}/Cairn" "${arm_binary}" "${x86_binary}"
    ;;
esac

chmod +x "${macos_dir}/Cairn"
...
icon_work_dir="$(mktemp -d "${TMPDIR:-/tmp}/cairn-icon.XXXXXX")"
cleanup() {
...
trap cleanup EXIT
iconset_dir="${icon_work_dir}/Cairn.iconset"
mkdir -p "${iconset_dir}"
