...
private let appAgentCommand = "__cairn-app-agent"
private let appAgentDisableEnvironmentKey = "CAIRN_DISABLE_APP_AGENT_PROXY"
...
enum MacOSAppAgentProxy {
    ...
    static func runAgent(arguments: [String]) throws {
        guard arguments.count == 2 else {
            throw CairnCLIError(message: "\(appAgentCommand) requires a socket path")
        }

        try MacOSAppAgentRuntime.run(socketPath: arguments[1])
    }

    static func shouldProxy(command: CairnCLICommand) -> Bool {
    ...
    @MainActor
    static func runProxy(command: CairnCLICommand, arguments: [String]) throws -> Int32 {
    ...
    private static var isRunningFromCairnAppBundle: Bool {
        Bundle.main.bundleURL.standardizedFileURL.pathExtension == "app"
            && PermissionSupport.isCairnBundleIdentifier(Bundle.main.bundleIdentifier)
    }

    private static var isRunningFromLaunchServicesAppInstance: Bool {
        isRunningFromCairnAppBundle && getppid() == 1
    }

    private static func defaultSocketPath() -> String {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("cairn-agent.sock")
            .standardizedFileURL
            .path
    }
    ...
    private static func connectOrLaunchAgent(socketPath: String) throws -> AppAgentSocketClient {
        guard let appURL = PermissionSupport.currentAppBundleURL() else {
            throw CairnCLIError(message: "Unable to locate Cairn.app for app-scoped macOS permissions.")
        }
        ...
        throw CairnCLIError(message: "Timed out waiting for Cairn.app agent to start.")
    }
...
private final class MacOSAppAgentRuntime: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        turnEndedObserver = DistributedNotificationCenter.default().addObserver(
            forName: cairnTurnEndedNotificationName,
            object: nil,
            queue: .main
        ) { _ in
            Task { @MainActor in
                resetCairnVisualCursor()
            }
        ...
        try withUnsafeMutablePointer(to: &address.sun_path) { pointer in
            try pointer.withMemoryRebound(to: CChar.self, capacity: pathCapacity) { buffer in
                ...
                guard bytes.count < pathCapacity else {
                    throw CairnCLIError(message: "Socket path is too long: \(path)")
                }
...
private final class AppAgentSocketClient: @unchecked Sendable {
    ...
    func request(_ object: [String: Any]) throws -> [String: Any] {
        ...
        guard let line = String(data: data, encoding: .utf8) else {
            throw CairnError.message("Failed to encode app-agent request.")
        }
        ...
        guard let responseLine = readAgentLine(file),
            ...
            throw CairnError.message("Cairn.app agent closed the connection.")
        }

        if let error = response["error"] as? String {
            throw CairnError.message(error)
        }
