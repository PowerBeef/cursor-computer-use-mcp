...
@main
enum CairnSmokeSuite {
    static func main() throws {
        let productsDirectory = try locateProductsDirectory()
        let fixtureURL = productsDirectory.appendingPathComponent("CairnFixture")
        let serverURL = productsDirectory.appendingPathComponent("Cairn")
        let appName = "CairnFixture"
    ...
    private static func runCursorIdleSmoke(serverURL: URL, appName: String) throws {
        ...
        environment["CAIRN_VISUAL_CURSOR"] = "1"
        environment["CAIRN_VISUAL_CURSOR_OBSERVATION_FILE"] = observationURL.path
    ...
    private static func smokeServerEnvironment() -> [String: String] {
        ...
        environment["CAIRN_DISABLE_APP_AGENT_PROXY"] = "1"
        return environment
    }

    private static func smokeFixtureEnvironment() -> [String: String] {
        var environment = ProcessInfo.processInfo.environment
        environment["CAIRN_FIXTURE_HEADLESS"] = "1"
        return environment
    }
    ...
    private static func cursorObservationFileURL() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("cairn-smoke", isDirectory: true)
