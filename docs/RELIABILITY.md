...
- Linux runtime *(upstream)*：`(cd apps/OpenComputerUseLinux && go test ./...)`、`./scripts/build-open-computer-use-linux.sh --arch arm64`
- 本地诊断：
  - `cairn doctor`
  - `cairn snapshot <app>`
...
- macOS 上必须给 `Cairn.app` 授权 `Accessibility` 与 `Screen Recording`；终端本身不应该再是必需授权对象。
...
1. 先跑 `cairn doctor`，确认权限状态；如果缺权限，命令会通过 `.app` app agent 拉起权限 onboarding 窗口，已全部授权则只打印状态并退出。
2. 用 `cairn list-apps` 确认目标 app 是否被发现。
3. 用 `cairn snapshot <app>` 看是 transport 问题还是 snapshot / action 问题。
4. 如果只想验证仓库基线，直接跑 fixture + smoke，不要先在复杂第三方 app 上排查。
5. 排查 Linux runtime 时，先确认目标命令是否由桌面用户运行，再用 `cairn call list_apps` 和 `cairn snapshot <app>` 区分 session/env 问题与 AT-SPI tree/action 问题。如果是 Codex MCP，重新执行 `cairn install-codex-mcp` 后重启 Codex，确认配置仍是 `cairn mcp`。
