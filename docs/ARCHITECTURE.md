...
> **Cairn:** Native macOS builds in this repository require **macOS 26 (Tahoe)+**. Cursor install, policy (`CairnPolicy`), and Composer workflow are documented in [FORK.md](FORK.md) and [CURSOR.md](CURSOR.md)—not repeated here.
...
- `apps/OpenComputerUseWindows` *(upstream-tracked)*  
  Experimental Windows runtime: Go CLI/MCP embeds a PowerShell UI Automation bridge; artifact `open-computer-use.exe` in npm `dist/windows/<arch>/`. Keeps the historical name to merge cleanly from upstream.
- `apps/OpenComputerUseLinux` *(upstream-tracked)*  
  Experimental Linux runtime: Go CLI/MCP embeds a Python AT-SPI bridge; artifact `open-computer-use` in npm `dist/linux/<arch>/`. Keeps the historical name to merge cleanly from upstream.
- `packages/CairnKit`  
  Core library: MCP stdio transport and tool registry, app discovery, Accessibility/window snapshot, input simulation, software cursor overlay, fixture test bridge, and **CairnPolicy** (Cursor fork).
...
- `skills/`  
  Agent-installable skills. `skills/open-computer-use/SKILL.md` is a light entry; details live in `references/`. `scripts/package-skill.sh` builds `.zip` / `.skill` artifacts. **Cursor:** `skills/cairn/`.
- `docs/`  
...
- On macOS, CLI commands (`mcp`, `doctor`, `call`, `snapshot`, `list-apps`) start the same `.app` agent via LaunchServices and a Unix domain socket under the user temp directory. Accessibility, ScreenCaptureKit, and tool execution run inside **Cairn.app**, not Terminal/Cursor/Node.
...
- When `CAIRN_VISUAL_CURSOR` is not disabled, `mcp` spins a minimal AppKit runtime (overlay on main thread, stdio server on a background thread).
...
- `get_app_state` returns tree text plus a screenshot MCP resource URI (`cairn://screenshot/latest`) unless `inline_image: true`. PNGs include **Set-of-Mark** numbered boxes aligned with `element_index`.
- `notifications/turn-ended` clears overlay state and in-process snapshot caches; CLI `cairn turn-ended` notifies a running AppKit MCP process (see `.cursor/hooks/turn-ended.example.json`).

### 3. Tool service

- `CairnService` maps tool requests to local capabilities; `CairnToolDispatcher` shares parsing between MCP and `cairn call`.
...
- `cairn call` supports single tools and `--calls` JSON arrays with shared in-process element index state; default 1s sleep between successful steps (`--sleep` override); stops on first `isError`.
- Password-manager denylist via `AppSafetyPolicy` / `CairnPolicy`; optional `.cursor/cairn-policy.json` allow/deny lists (fork).
...
- **Non-intrusive first:** AX actions, settable checks for `set_value`, semantic click fallbacks, coordinate hit-test, `CGEvent.postToPid` for keys/scroll; global pointer fallbacks only with `CAIRN_ALLOW_GLOBAL_POINTER_FALLBACKS=1`.

### 4. Fixture bridge

- `CairnFixture` writes window/element state to a temp JSON file.
...
### 6. Windows runtime *(upstream)*

- `apps/OpenComputerUseWindows`: Go CLI, MCP, embedded `runtime.ps1`, UIA patterns with Win32 message fallbacks.
- Opt-in: `OPEN_COMPUTER_USE_WINDOWS_ALLOW_APP_LAUNCH`, `ALLOW_FOCUS_ACTIONS`, `ALLOW_UIA_TEXT_FALLBACK` (env names retain the upstream prefix).
...
- Official bundled `computer-use` may be killed by launch constraints; use normal Codex chains for official tools, `cairn mcp` for this project.
- Permission onboarding and visual cursor are close but not identical to official embedded choreography.
- Screenshots: ScreenCaptureKit → MCP resource `cairn://screenshot/latest` (size-capped PNG) by default; pass `inline_image: true` to `get_app_state` to embed an inline `image` content block. PNG bytes live in memory, not repo temp files; coordinate tools map pixel size to window points.
...
- App package: `./scripts/build-cairn-app.sh debug`
...
- Diagnostics: `cairn doctor`, `snapshot`, `call list_apps`, `call --calls '[...]'`
