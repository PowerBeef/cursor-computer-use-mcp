...
Headless harness under `benchmarks/` measures the native `cairn mcp` server. An optional legacy comparison against `npx computer-use-mcp` is available.

## Prerequisites

- **macOS 26 (Tahoe)+** for this fork’s native macOS build
- Accessibility + Screen Recording granted for **Cairn.app**
- `cairn` on `PATH` (from `npm run npm:build` or `npm i -g cairn-computer-use`)
- TextEdit available for typing tasks

## macOS 26 validation (2026-05-27)

Validated on Tahoe host (darwin/arm64, global `cairn`):
...
| `screenshot_latency` | `get_app_state` capture latency + screenshot bytes (inline `image` or `resources/read` on `cairn://screenshot/latest`) |
...
| `BENCHMARK_OCU_COMMAND` | `cairn` | Override OCU binary path |
