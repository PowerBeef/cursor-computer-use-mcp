...
final class CairnKitTests: XCTestCase {
    func testCLIRecognizesGlobalHelpAndVersionFlags() throws {
        XCTAssertEqual(try parseCairnCLI(arguments: ["-h"]), .help(command: nil))
        XCTAssertEqual(try parseCairnCLI(arguments: ["--help"]), .help(command: nil))
        XCTAssertEqual(try parseCairnCLI(arguments: ["-v"]), .version)
        XCTAssertEqual(try parseCairnCLI(arguments: ["--version"]), .version)
    }

    func testCLIRecognizesCommandSpecificHelp() throws {
        XCTAssertEqual(try parseCairnCLI(arguments: ["help", "snapshot"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseCairnCLI(arguments: ["snapshot", "--help"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseCairnCLI(arguments: ["doctor", "-h"]), .help(command: "doctor"))
        XCTAssertEqual(try parseCairnCLI(arguments: ["doctor", "--cursor"]), .doctor(cursor: true))
        XCTAssertEqual(try parseCairnCLI(arguments: ["doctor"]), .doctor(cursor: false))
        XCTAssertEqual(try parseCairnCLI(arguments: ["call", "--help"]), .help(command: "call"))
    }

    func testCLIRecognizesSingleToolCallCommand() throws {
        XCTAssertEqual(
            try parseCairnCLI(arguments: ["call", "list_apps"]),
            .call(.single(toolName: "list_apps", argumentsJSON: nil, argumentsFile: nil))
        )

        XCTAssertEqual(
            try parseCairnCLI(arguments: ["call", "get_app_state", "--args", #"{"app":"TextEdit"}"#]),
    ...
    func testCLIRecognizesJSONSequenceCallCommand() throws {
        ...
        XCTAssertEqual(
            try parseCairnCLI(arguments: ["call", "--calls", calls]),
            .call(.sequence(
                callsJSON: calls,
                callsFile: nil,
                interCallDelay: cairnDefaultInterCallDelay
            ))
        )
    }
    ...
    func testCLIRecognizesJSONSequenceCallCommandWithCustomSleep() throws {
        ...
        XCTAssertEqual(
            try parseCairnCLI(arguments: ["call", "--calls", calls, "--sleep", "0.5"]),
    ...
    func testCLIRecognizesTurnEndedNotifyPayload() throws {
        ...
        XCTAssertEqual(try parseCairnCLI(arguments: ["turn-ended"]), .turnEnded(payload: nil))
        XCTAssertEqual(try parseCairnCLI(arguments: ["turn-ended", payload]), .turnEnded(payload: payload))
        XCTAssertEqual(
            try parseCairnCLI(arguments: ["turn-ended", "--previous-notify", #"["/bin/true"]"#, payload]),
            .turnEnded(payload: payload)
        )
    }

    func testCLIRequiresSnapshotArgument() {
        XCTAssertThrowsError(try parseCairnCLI(arguments: ["snapshot"])) { error in
            XCTAssertEqual(
                error as? CairnCLIError,
                CairnCLIError(
    ...
    func testCLIRejectsMixedCallSequenceInputs() {
        XCTAssertThrowsError(try parseCairnCLI(arguments: ["call", "list_apps", "--calls", "[]"])) { error in
            XCTAssertEqual(
                error as? CairnCLIError,
                CairnCLIError(
    ...
    func testCLIRejectsSleepForSingleToolCall() {
        XCTAssertThrowsError(try parseCairnCLI(arguments: ["call", "list_apps", "--sleep", "0.5"])) { error in
            XCTAssertEqual(
                error as? CairnCLIError,
                CairnCLIError(
    ...
    func testCLIRejectsInvalidSequenceSleepValue() {
        XCTAssertThrowsError(try parseCairnCLI(arguments: ["call", "--calls", "[]", "--sleep", "-1"])) { error in
            XCTAssertEqual(
                error as? CairnCLIError,
                CairnCLIError(
    ...
    func testCLIRejectsUnknownOption() {
        XCTAssertThrowsError(try parseCairnCLI(arguments: ["--verbose"])) { error in
            XCTAssertEqual(
                error as? CairnCLIError,
                CairnCLIError(
    ...
    func testGeneralHelpListsCommandsAndGlobalFlags() {
        let help = cairnHelpText()

        XCTAssertTrue(help.contains("cairn [command] [options]"))
    ...
    func testResolvedVersionFallsBackWhenBundleHasNoVersionMetadata() {
        XCTAssertEqual(resolvedCairnVersion(bundle: Bundle(for: Self.self)), cairnVersion)
    }
    ...
    func testReadToolArgumentsRejectsNonObject() {
        XCTAssertThrowsError(try readCairnToolArguments(json: #"["TextEdit"]"#, file: nil)) { error in
            XCTAssertEqual(
                error as? CairnCLIError,
                CairnCLIError(message: "--args must be a JSON object", helpCommand: "call")
            )
        }
    }

    func testReadCallSequenceAcceptsJSONArrays() throws {
        let calls = try readCairnCallSequence(
    ...
    func testRunCallSequenceSleepsBetweenSuccessfulOperations() throws {
        ...
        XCTAssertEqual(recordedSleeps, [cairnDefaultInterCallDelay, cairnDefaultInterCallDelay])
    ...
    func testMacOSAppAgentProxyDecisionKeepsNonAutomationCommandsLocal() {
        for command in [
            CairnCLICommand.turnEnded(payload: nil),
            .help(command: nil),
            .version,
        ] {
    ...
    func testPreferredPermissionAppBundleURLPrefersInstalledCopyOverTransientRunningCopy() {
        let installed = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/cairn/dist/Cairn.app")
        let running = URL(fileURLWithPath: "/Users/example/projects/open-codex-computer-use/dist/Cairn.app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/open-codex-computer-use-debug/dist/Cairn.app")
    ...
    func testPreferredPermissionAppBundleURLPrefersRunningDevelopmentCopy() {
        let installed = URL(fileURLWithPath: "/Applications/Cairn.app")
        let running = URL(fileURLWithPath: "/Users/example/projects/open-codex-computer-use/dist/Cairn (Dev).app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/open-codex-computer-use-debug/dist/Cairn (Dev).app")
    ...
    func testPreferredPermissionAppBundleURLCanPreferRunningReleaseCopyOverStaleInstalledCopy() {
        let staleInstalled = URL(fileURLWithPath: "/Users/example/projects/open-codex-computer-use/dist/npm/cairn/dist/Cairn.app")
        let running = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/cairn/dist/Cairn.app")
    ...
    func testPreferredInstalledAppBundleURLUsesFirstDiscoveredInstalledCopy() {
        let applications = URL(fileURLWithPath: "/Applications/Cairn.app")
        let npm = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/cairn/dist/Cairn.app")
        let duplicateApplications = URL(fileURLWithPath: "/Applications/Cairn.app")
    ...
    func testInitializeResponseContainsToolsCapability() throws {
        let server = StdioMCPServer(service: CairnService())
        ...
        XCTAssertTrue(response!.contains(#""name":"cairn""#))
    ...
    func testMCPResourcesListExposesLatestScreenshotURI() throws {
        ...
        XCTAssertTrue(resources.contains { ($0["uri"] as? String)?.hasPrefix("cairn://screenshot/") == true })
    }
    ...
    func testMCPResourcesReadReturnsStoredPNG() throws {
        ...
        let server = StdioMCPServer()
        let response = try XCTUnwrap(
            server.handle(line: #"{"jsonrpc":"2.0","id":4,"method":"resources/read","params":{"uri":"cairn://screenshot/latest"}}"#)
        )
        let payload = try XCTUnwrap(JSONSerialization.jsonObject(with: Data(response.utf8)) as? [String: Any])
        let result = try XCTUnwrap(payload["result"] as? [String: Any])
        let contents = try XCTUnwrap(result["contents"] as? [[String: Any]])
        let blob = try XCTUnwrap(contents.first?["blob"] as? String)
        XCTAssertEqual(uri, "cairn://screenshot/latest")
    ...
    func testMCPTurnEndedClearsScreenshotResource() throws {
        ...
        XCTAssertNil(MCPScreenshotResourceStore.shared.pngData(for: "cairn://screenshot/latest"))
    }
    ...
    func testDispatcherMissingArgumentsMatchOfficialToolText() {
        let dispatcher = CairnToolDispatcher()
    ...
    func testBlockingAsyncBridgeTimesOutScreenshotWork() {
        XCTAssertThrowsError(
            ...
            XCTAssertTrue(
                (error as? CairnError)?.errorDescription?.contains("timed out") == true
            )
        }
    }

    func testCairnErrorsFormatLikeToolText() {
        XCTAssertEqual(CairnError.appNotFound("Sublime Text").errorDescription, #"appNotFound("Sublime Text")"#)
        XCTAssertTrue(CairnError.appNotFound("Sublime Text").toolResultIsError)
        XCTAssertTrue(CairnError.invalidArguments("bad").toolResultIsError)
    }
    ...
    func testNoWindowErrorMessageMatchesOfficialShape() {
        ...
        XCTAssertEqual(
            CairnError.stateUnavailable(computerUseNoWindowFoundMessage).errorDescription,
    ...
    func testCairnPolicyBlocksPasswordManagersByDefault() {
        XCTAssertTrue(CairnPolicy.isBlocked(bundleIdentifier: "com.1password.1password"))
        XCTAssertTrue(CairnPolicy.isBlocked(query: "com.bitwarden.desktop"))
    }

    func testMCPTextTruncationAddsSuffix() {
        let long = String(repeating: "a", count: 100)
        let truncated = MCPTextTruncation.truncate(long, environment: ["CAIRN_MAX_TEXT_CHARS": "20"])
    ...
    func testVisualCursorEnvFlagDefaultsToEnabled() {
        ...
        XCTAssertTrue(visualCursorEnabled(environment: ["CAIRN_VISUAL_CURSOR": "1"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["CAIRN_VISUAL_CURSOR": "0"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["CAIRN_VISUAL_CURSOR": "false"]))
    }

    func testInputFallbackDebugFlagDefaultsToDisabled() {
        XCTAssertFalse(inputFallbackDebugEnabled(environment: [:]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["CAIRN_DEBUG_INPUT_FALLBACKS": "1"]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["CAIRN_DEBUG_INPUT_FALLBACKS": "true"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["CAIRN_DEBUG_INPUT_FALLBACKS": "0"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["CAIRN_DEBUG_INPUT_FALLBACKS": "off"]))
    }

    func testGlobalPointerFallbackFlagDefaultsToDisabled() {
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: [:]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["CAIRN_ALLOW_GLOBAL_POINTER_FALLBACKS": "1"]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["CAIRN_ALLOW_GLOBAL_POINTER_FALLBACKS": "yes"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["CAIRN_ALLOW_GLOBAL_POINTER_FALLBACKS": "0"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["CAIRN_ALLOW_GLOBAL_POINTER_FALLBACKS": "false"]))
    }
    ...
    func testSetValueAttributeGateMatchesOfficialSettableBoundary() throws {
        ...
        XCTAssertThrowsError(
            ...
            XCTAssertEqual(
                (error as? CairnError)?.errorDescription,
