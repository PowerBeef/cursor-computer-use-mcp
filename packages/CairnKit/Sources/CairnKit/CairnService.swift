...
func inputFallbackDebugEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["CAIRN_DEBUG_INPUT_FALLBACKS"]?
...
func globalPointerFallbacksEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["CAIRN_ALLOW_GLOBAL_POINTER_FALLBACKS"]?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    else {
        return false
...
func setValueAttributeIsSettable(result: AXError, settable: Bool, attribute: String) throws -> Bool {
    guard result == .success else {
        throw CairnError.message("AXUIElementIsAttributeSettable(\(attribute)) failed with \(result.rawValue)")
    }
...
public final class CairnService {
    private static let instanceLock = NSLock()
    private nonisolated(unsafe) static let activeInstances = NSHashTable<CairnService>.weakObjects()
    ...
    public func click(
        ...
        if snapshot.mode == .fixture {
            ...
            if let elementIndex {
                ...
                guard let identifier = record.identifier else {
                    throw CairnError.invalidArguments("fixture click requires an identifier-backed element")
                }
                ...
                throw CairnError.invalidArguments("click requires either element_index or x/y")
            }
        ...
        if let elementIndex {
            ...
            guard let targetPoint = try globalClickPoint(for: record, snapshot: snapshot) else {
                throw CairnError.stateUnavailable("element \(elementIndex) has no clickable frame")
            }
    ...
    public func performSecondaryAction(
        ...
        if snapshot.mode == .fixture {
            guard action.caseInsensitiveCompare("Raise") == .orderedSame else {
                throw CairnError.message(invalidSecondaryActionMessage(action: action, record: record))
            }
        ...
        guard let element = record.element else {
            throw CairnError.stateUnavailable("element \(elementIndex) has no backing accessibility object")
        }

        let result = AXUIElementPerformAction(element, rawAction as CFString)
        guard result == .success else {
            throw CairnError.message("AXUIElementPerformAction failed with \(result.rawValue)")
        }
    ...
    public func scroll(
        ...
        guard ["up", "down", "left", "right"].contains(normalized) else {
            throw CairnError.message("Invalid scroll direction: \(direction)")
        }
        guard pages.isFinite, pages > 0 else {
            throw CairnError.message("pages must be > 0")
        }
        ...
        if snapshot.mode == .fixture {
            guard let identifier = record.identifier else {
                throw CairnError.invalidArguments("fixture scroll requires an identifier-backed element")
            }
        ...
        if let repeatCount = integralScrollPageCount(pages),
           ...
           let element = record.element {
            ...
            throw CairnError.stateUnavailable("element \(elementIndex) has no scrollable frame")
        }
    ...
    public func typeText(app query: String, text: String, includeScreenshot: Bool = false) throws -> ToolCallResult {
        ...
        guard try canTypeTextUsingKeyboardFallback(focusedElement: focusedElement) else {
            throw CairnError.stateUnavailable("type_text requires a focused editable text element. Click a text entry area first, or use set_value on a settable text element.")
        }
    ...
    public func setValue(
        ...
        if snapshot.mode == .fixture {
            guard let identifier = record.identifier else {
                throw CairnError.invalidArguments("fixture set_value requires a known element identifier")
            }
        ...
        guard try isSettableForSetValue(element: element, attribute: kAXValueAttribute) else {
            throw CairnError.message(nonSettableSetValueErrorMessage)
        }
        ...
        do {
            ...
            guard result == .success else {
                throw CairnError.message("AXUIElementSetAttributeValue failed with \(result.rawValue)")
            }
    ...
    private func lookupElement(snapshot: AppSnapshot, index: String) throws -> ElementRecord {
        ...
        guard let parsedIndex = Int(trimmed), let record = snapshot.elements[parsedIndex] else {
            throw CairnError.invalidArguments(
    ...
    private func selectContainingListItem(for element: AXUIElement) throws -> Bool {
        ...
        case .failure, .attributeUnsupported, .actionUnsupported, .cannotComplete, .noValue, .invalidUIElement, .illegalArgument:
            return false
        default:
            throw CairnError.message("AXUIElementSetAttributeValue(\(kAXSelectedChildrenAttribute)) failed with \(result.rawValue)")
        }
    }
    ...
    private func performAction(named action: String, on element: AXUIElement, availableActions: [String], repeatCount: Int = 1) throws -> Bool {
        ...
        for index in 0..<attempts {
            ...
            case .failure, .actionUnsupported, .attributeUnsupported, .cannotComplete, .noValue, .invalidUIElement, .illegalArgument:
                return false
            default:
                throw CairnError.message("AXUIElementPerformAction(\(action)) failed with \(result.rawValue)")
            }
        }
    ...
    private func setBoolAttribute(named attribute: String, on element: AXUIElement) throws -> Bool {
        ...
        case .failure, .attributeUnsupported, .actionUnsupported, .cannotComplete, .noValue, .invalidUIElement, .illegalArgument:
            return false
        default:
            throw CairnError.message("AXUIElementSetAttributeValue(\(attribute)) failed with \(result.rawValue)")
        }
    }
    ...
    private func windowPointToGlobalPoint(snapshot: AppSnapshot, point: CGPoint) throws -> CGPoint {
        guard let windowBounds = snapshot.windowBounds else {
            ...
            throw CairnError.stateUnavailable("No window bounds are available for \(appReference). Run get_app_state after bringing the app on screen.")
        }
    ...
    private func fixtureIdentifier(at point: CGPoint, snapshot: AppSnapshot) throws -> String {
        ...
        guard let identifier = candidates.first?.identifier else {
            throw CairnError.invalidArguments("No fixture element contains coordinate (\(Int(point.x)), \(Int(point.y)))")
        }
    ...
    private func debugInputFallback(tool: String, targetDescription: String, snapshot: AppSnapshot) {
        ...
        let appReference = snapshot.app.bundleIdentifier ?? snapshot.app.name
        fputs(
            "[cairn] global pointer fallback tool=\(tool) app=\(appReference) target=\(targetDescription)\n",
            stderr
        )
    }
    ...
    private func debugClickDecision(_ message: String) {
        ...
        fputs("[cairn] click decision \(message)\n", stderr)
    }
    ...
    private func performNonAXClickFallback(
        ...
        do {
            ...
            guard globalPointerFallbacksEnabled(environment: ProcessInfo.processInfo.environment) else {
                throw CairnError.message(
                    "click could not be handled through accessibility, and global pointer fallback is disabled. Set CAIRN_ALLOW_GLOBAL_POINTER_FALLBACKS=1 to allow physical-pointer fallback for this process."
                )
            }
        }
    }
