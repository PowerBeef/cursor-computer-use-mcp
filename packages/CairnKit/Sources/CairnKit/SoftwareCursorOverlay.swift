...
func visualCursorEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["CAIRN_VISUAL_CURSOR"]?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() else {
        return true
    }
...
public func visualCursorObservationFileURL(environment: [String: String]) -> URL? {
    guard
        let rawPath = environment["CAIRN_VISUAL_CURSOR_OBSERVATION_FILE"]?
...
public let cairnTurnEndedNotificationName = Notification.Name("com.powerbeef.cairn.turn-ended")

public func postCairnTurnEndedNotification() {
    DistributedNotificationCenter.default().postNotificationName(
        cairnTurnEndedNotificationName,
        object: nil,
        userInfo: nil,
...
@MainActor
public func resetCairnVisualCursor() {
