...
enum SnapshotBuilder {
    static func build(
        ...
        guard permissions.accessibilityTrusted else {
            throw CairnError.permissionDenied("Accessibility permission is required. Run `cairn doctor` and grant access to Cairn.")
        }
        ...
        guard let resolvedFocusedWindow = focusedWindow else {
            throw CairnError.stateUnavailable(computerUseNoWindowFoundMessage)
        }
...
private struct WindowCapture {
    ...
    private static func debugScreenshotCapture(_ message: String) {
        ...
        fputs("[cairn] screenshot \(message)\n", stderr)
    }
...
enum BlockingAsyncBridge {
    ...
    static func run<T>(timeout: TimeInterval? = nil, _ operation: @escaping @Sendable () async throws -> T) throws -> T {
        ...
        guard waitForSignal(semaphore, timeout: timeout) else {
            ...
            throw CairnError.message("ScreenCaptureKit screenshot task timed out after \(timeout ?? 0) seconds.")
        }

        return try resultBox.result?.get() ?? {
            throw CairnError.message("ScreenCaptureKit screenshot task finished without producing a result.")
        }()
    }
