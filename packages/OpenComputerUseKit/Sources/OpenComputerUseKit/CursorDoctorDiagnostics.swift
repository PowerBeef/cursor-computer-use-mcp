...
public struct CursorDoctorDiagnostics: Sendable {
    ...
    public let openComputerUseOnPath: Bool
    ...
    public static func run(cursorMode: Bool) -> CursorDoctorDiagnostics {
        ...
        let whichPath = runWhich("open-computer-use")
        ...
        return CursorDoctorDiagnostics(
            permissionSummary: permissions.summary + " Grant Accessibility and Screen Recording to Open Computer Use.app (not Terminal/Cursor).",
            macOSVersionOK: macOSOK,
            macOSVersionLine: versionLine + (macOSOK ? "" : " — upgrade required for this repository's native build."),
            openComputerUseOnPath: onPath,
            pathLine: onPath
                ? "PATH: open-computer-use found at \(whichPath!)"
                : "PATH: open-computer-use not found (required for Cursor Automations and MCP).",
    ...
    private static func scanCursorMCPConfigs() -> [String] {
        ...
        for path in paths {
            ...
            if servers["computer-use-mcp"] != nil {
                warnings.append("MCP (\(path.lastPathComponent)): disable legacy computer-use-mcp (single computer tool). Use cursor-computer-use (9 tools).")
            }
            ...
            if computerUseEntries.count > 1 {
                warnings.append("MCP (\(path.lastPathComponent)): multiple Computer Use servers enabled (\(computerUseEntries.joined(separator: ", "))). Enable only cursor-computer-use.")
            }

            if servers["cursor-computer-use"] == nil, path.lastPathComponent == "mcp.json" {
                warnings.append("MCP: cursor-computer-use not configured in \(path.path). Run open-computer-use install-cursor-mcp.")
            }
        }
