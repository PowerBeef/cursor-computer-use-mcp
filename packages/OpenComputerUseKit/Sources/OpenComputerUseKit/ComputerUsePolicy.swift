...
private struct CursorComputerUsePolicyFile: Decodable {
...
enum ComputerUsePolicy {
    private static let userPolicyPath = FileManager.default.homeDirectoryForCurrentUser
        .appendingPathComponent(".cursor/computer-use-policy.json")
    private static let projectPolicyFileName = "computer-use-policy.json"
    ...
    private static func loadPolicy() -> LoadedPolicy {
        var merged = CursorComputerUsePolicyFile(
        ...
        for path in policyPaths() {
            guard let data = try? Data(contentsOf: path),
                  let file = try? JSONDecoder().decode(CursorComputerUsePolicyFile.self, from: data)
            else {
                continue
            }
...
enum MCPTextTruncation {
    ...
    private static func maxChars(from environment: [String: String]) -> Int {
        guard let raw = environment["OPEN_COMPUTER_USE_MAX_TEXT_CHARS"]?
